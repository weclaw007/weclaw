...
class Doctor:
    ...
    def check_chocolatey(self) -> CheckResult:
        ...
        if choco_path:
            ...
            try:
                ...
                print_success("已安装")
                return CheckResult(name=name, passed=True, message=f"{name}: {version} ({choco_path})")
            except Exception:
                print_success("已安装")
                return CheckResult(name=name, passed=True, message=f"{name} 已安装: {choco_path}")
        ...
        try:
            ...
            if success:
                ...
                return CheckResult(name=name, passed=True, message=f"{name}: 已自动安装成功", auto_installed=True)
            else:
                print_fail(f"{name} 自动安装失败")
                return CheckResult(
                    name=name, passed=False, message=f"{name}: 自动安装失败",
                    hint="请以管理员权限运行或手动安装: https://chocolatey.org/install",
                )
        except Exception as e:
            print_fail(f"{name} 自动安装失败 ({e})")
            return CheckResult(
                name=name, passed=False, message=f"{name}: 自动安装失败 ({e})",
                hint="请以管理员权限运行或手动安装: https://chocolatey.org/install",
            )
    ...
    def check_homebrew(self) -> CheckResult:
        ...
        if brew_path:
            ...
            try:
                ...
                print_detail(f"检测到版本: {version}")
                print_success("已安装")
                return CheckResult(name=name, passed=True, message=f"{name}: {version}")
            except Exception:
                print_success("已安装")
                return CheckResult(name=name, passed=True, message=f"{name} 已安装: {brew_path}")
    ...
    def check_uv(self) -> CheckResult:
        ...
        if uv_path:
            ...
            try:
                ...
                print_success("已安装")
                return CheckResult(name=name, passed=True, message=f"{name} 已安装: {uv_path}")
    ...
    def check_node(self) -> CheckResult:
        ...
        if node_path:
            ...
            try:
                ...
                print_success("已安装")
                return CheckResult(name=name, passed=True, message=f"{name} 已安装: {node_path}")
    ...
    def check_go(self) -> CheckResult:
        ...
        if go_path:
            ...
            try:
                ...
                print_success("已安装")
                return CheckResult(name=name, passed=True, message=f"{name} 已安装: {go_path}")
    ...
    def check_go_path(self) -> CheckResult:
        ...
        if in_path:
            ...
            return CheckResult(name=name, passed=True, message=f"{name}: {go_bin_dir} 已在 PATH 中")
        ...
        if fix_result:
            ...
            if fix_result == "written":
                print_success("已自动添加到 shell 配置文件并更新当前环境")
                return CheckResult(
                    name=name, passed=True,
                ...
                print_success("shell 配置文件中已有配置，已更新当前环境")
                return CheckResult(
                    name=name, passed=True,
    ...
    def print_report(self) -> int:
        ...
        print(f"\n  {BOLD}{'─' * 40}{RESET}")
        total = passed + failed
        summary_parts = [f"{GREEN}{passed} 项通过{RESET}"]
        if auto_installed > 0:
